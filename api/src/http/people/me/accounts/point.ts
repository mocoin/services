...
export async function search(event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> {
    try {
        ...
        accountOwnershipInfos = accountOwnershipInfos.filter((o) => o.typeOfGood.accountType === mocoin.factory.accountType.Point);
        let accounts: mocoin.factory.pecorino.account.IAccount<mocoin.factory.accountType.Point>[] = [];
        if (accountOwnershipInfos.length > 0) {
            ...
            accounts = await accountService.search<mocoin.factory.accountType.Point>({
                accountType: mocoin.factory.accountType.Point,
...
export async function searchMoneyTransferActions(event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> {
    try {
        ...
        const actions = await accountService.searchMoneyTransferActions<mocoin.factory.accountType.Point>({
            accountType: mocoin.factory.accountType.Point,
...
export async function open(event: APIGatewayEvent, context: Context): Promise<APIGatewayProxyResult> {
    try {
        ...
        const account = await mocoin.service.account.point.open({
            name: body.name
        })({
        ...
        const ownershipInfo: mocoin.factory.ownershipInfo.IOwnershipInfo<mocoin.factory.ownershipInfo.AccountGoodType.Account>
            ...
            // 十分にユニーク
            identifier: `${sub}-${goodType}-${mocoin.factory.accountType.Point}-${account.accountNumber}`,
            typeOfGood: {
                typeOf: goodType,
                accountType: mocoin.factory.accountType.Point,
